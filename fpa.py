...
with st.sidebar:
    ...
    export_prefix = st.text_input("Prefixo arquivo", value="BASE_Completo", key="export_prefix")
    ...
    st.divider()
    st.header("🎯 Meta")
    meta_conv = st.slider("% Convergência (objetivo)", min_value=0, max_value=100, value=80, step=1)

# Paleta aplicada
COLOR_MAP = {"SETOR CONVERGENTE": col_ok, "SETOR DIVERGENTE": col_div, "**SEM RELAÇÃO**": col_sr}
...
def safe_name(s: str) -> str: return "".join(ch for ch in s if ch not in r'\/:*?"<>|').strip()
def norm_cols(df: pd.DataFrame) -> pd.DataFrame: df.columns = [str(c).strip() for c in df.columns]; return df
def norm(x): 
...
def insert_after(df: pd.DataFrame, after_col: str, new_col: str, values) -> pd.DataFrame:
    cols = list(df.columns); idx = cols.index(after_col)
...
def load_processing_file(proc_dir: Path) -> Path:
    if not proc_dir.exists(): raise FileNotFoundError(f"Pasta de processamento inexistente: {proc_dir}")
    files = sorted(proc_dir.glob("*.xlsx"), key=lambda x: x.stat().st_mtime, reverse=True)
    if not files: raise FileNotFoundError("Nenhum arquivo encontrado em Input/Processamento.")
    f = files[0]
...
def load_premissas(prem_dir: Path) -> Tuple[Path, Path]:
    ...
    if not depara.exists(): raise FileNotFoundError(f"Premissa ausente: {depara}")
    if not check.exists():  raise FileNotFoundError(f"Premissa ausente: {check}")
    return depara, check

def processar(df_base: pd.DataFrame, map_df: pd.DataFrame, chk_df: pd.DataFrame) -> pd.DataFrame:
    df_base = norm_cols(df_base); map_df = norm_cols(map_df); chk_df = norm_cols(chk_df)
    # De_Para
    ds_col = find_col(df_base.columns, ["ds_pro_fat", "DS_PRO_FAT", "ds pro fat"])
    if ds_col is None: raise KeyError("Coluna 'ds_pro_fat' não encontrada na aba Base.")
    ds_map = find_col(map_df.columns, ["ds_pro_fat", "DS_PRO_FAT", "ds pro fat"])
    para_col = find_col(map_df.columns, ["Para", "para", "PARA"])
    if ds_map is None or para_col is None: raise KeyError("Premissa De_Para sem 'ds_pro_fat' e/ou 'Para'.")
    mapping = { norm(r[ds_map]): r[para_col] for _, r in map_df.iterrows() if norm(r[ds_map]) }
    diaria = df_base[ds_col].map(norm).map(lambda x: mapping.get(x, "**SEM RELAÇÃO**"))
    df1 = insert_after(df_base, ds_col, "Diária Padrão", diaria)
    # Base_Check
    diaria_chk = find_col(chk_df.columns, ["Diária Padrão","Diaria Padrao","Diaria Padrão"])
    setor_chk  = find_col(chk_df.columns, ["Setor Padrão","Setor Padrao"])
    if diaria_chk is None or setor_chk is None: raise KeyError("Premissa Base_Check sem 'Diária Padrão' e/ou 'Setor Padrão'.")
    chk_map = { norm(r[diaria_chk]): r[setor_chk] for _, r in chk_df.iterrows() if norm(r[diaria_chk]) }
    setor_padrao = df1["Diária Padrão"].map(lambda x: chk_map.get(x, "**SEM RELAÇÃO**"))
    df2 = insert_after(df1, "Diária Padrão", "Setor Padrão", setor_padrao)
    # Check
    setor_orig_col = find_col(df2.columns, ["ds_grupo_macro","Setor","setor","Setor Original","setor original","DS_GRUPO_MACRO"])
    def _check(row):
        sp = norm(row["Setor Padrão"])
        if sp == "**SEM RELAÇÃO**": return "**SEM RELAÇÃO**"
        if setor_orig_col:
            so = norm(row[setor_orig_col]); return "SETOR CONVERGENTE" if so == sp else "SETOR DIVERGENTE"
...
def to_excel_bytes_clean(df: pd.DataFrame, sheet_name: str) -> bytes:
    ...
    bio.seek(0)
    wb = load_workbook(bio); ws = wb[sheet_name]
    mr, mc = ws.max_row, ws.max_column
    empty_border = Border(); empty_fill = PatternFill(fill_type=None)
    for row in ws.iter_rows(min_row=1, max_row=mr, min_col=1, max_col=mc):
        for cell in row:
            cell.font = Font(name="Arial", size=9, bold=False); cell.border = empty_border; cell.fill = empty_fill
    for cell in ws[1]: cell.font = Font(name="Arial", size=9, bold=True)
...
def try_export_pdf(html_bytes: bytes) -> Optional[bytes]:
    try:
        ...
        with tempfile.NamedTemporaryFile(suffix=".html", delete=False) as f:
            f.write(html_bytes); f.flush(); hp = f.name
        pdf_bytes = pdfkit.from_file(hp, False); os.remove(hp); return pdf_bytes
    except Exception:
        pass
    try:
...
try:
    ...
    if logo_path.exists():
        ...
        with c1: st.image(str(logo_path))
        with c2: 
            st.markdown("### Painel — Produção de Diárias")
            st.caption(proc_file.name)

    # ===== Detectar colunas =====
    col_valor = find_col(df_out.columns, ["PRODr","PROD","Valor","valor","vr_prod","vr_total"])
    if col_valor is None: raise KeyError("Coluna de valores não encontrada (ex.: 'PRODr').")
    col_setor_macro = find_col(df_out.columns, ["ds_grupo_macro","DS_GRUPO_MACRO","Setor","setor"])
    if col_setor_macro is None: raise KeyError("Coluna de agrupamento setorial não encontrada (ex.: 'ds_grupo_macro').")
    col_diaria = "Diária Padrão"; col_check = "Check"

    # ===== Período (opcional) =====
    col_data = find_col(df_out.columns, ["data","dt","Data","DT","competencia","Competência","PERIODO","Período","periodo"])
    dfv = df_out.copy()
    if col_data:
        dfv["_data"] = pd.to_datetime(df_out[col_data], errors="coerce")
        mind, maxd = pd.to_datetime(dfv["_data"].min()), pd.to_datetime(dfv["_data"].max())
        with st.sidebar:
            st.divider(); st.header("🗓️ Período")
            d_ini, d_fim = st.date_input("Intervalo", value=(mind.date(), maxd.date()))
        dfv = dfv[(dfv["_data"].dt.date >= d_ini) & (dfv["_data"].dt.date <= d_fim)]
    ...
    # ===== Drill-down filters =====
    st.markdown("---"); st.subheader("🎯 Drill-down")
    f1, f2, f3 = st.columns(3)
    with f1:
        opt_setor = ["(Todos)"] + sorted(dfv["Setor Padrão"].dropna().astype(str).unique().tolist())
        sel_setor = st.selectbox("Setor Padrão", opt_setor, index=0)
    df_d1 = dfv if sel_setor == "(Todos)" else dfv[dfv["Setor Padrão"].astype(str) == sel_setor]

    with f2:
        opt_diaria = ["(Todos)"] + sorted(df_d1["Diária Padrão"].dropna().astype(str).unique().tolist())
        sel_diaria = st.selectbox("Diária Padrão", opt_diaria, index=0)
    df_d2 = df_d1 if sel_diaria == "(Todos)" else df_d1[df_d1["Diária Padrão"].astype(str) == sel_diaria]

    with f3:
        if "ds_pro_fat" in df_d2.columns:
            opt_ds = ["(Todos)"] + sorted(df_d2["ds_pro_fat"].dropna().astype(str).unique().tolist())
        else:
            opt_ds = ["(N/D)"]
        sel_ds = st.selectbox("ds_pro_fat", opt_ds, index=0)
    df_d3 = df_d2 if (sel_ds in ["(Todos)","(N/D)"]) else df_d2[df_d2["ds_pro_fat"].astype(str) == sel_ds]

    # ===== KPIs =====
    df_dash = df_d3.copy()
    df_dash["_valor"] = pd.to_numeric(df_dash[col_valor], errors="coerce").fillna(0)
    tot = float(df_dash["_valor"].sum())
    conv = float(df_dash.loc[df_dash[col_check]=="SETOR CONVERGENTE","_valor"].sum())
    div  = float(df_dash.loc[df_dash[col_check]=="SETOR DIVERGENTE","_valor"].sum())
    sr   = float(df_dash.loc[(df_dash[col_diaria]=="**SEM RELAÇÃO**") | (df_dash["Setor Padrão"]=="**SEM RELAÇÃO**"), "_valor"].sum())
    pct = lambda x: 0 if tot==0 else x/tot*100
    pct_conv = pct(conv)

    kc1,kc2,kc3,kc4 = st.columns(4)
    kc1.markdown(f"<div class='kpi'><div class='title'>Total (∑ PRODr)</div><div class='value'>{tot:,.2f}".replace(",", "X").replace(".", ",").replace("X",".") + "</div></div>", unsafe_allow_html=True)
    kc2.markdown(f"<div class='kpi'><div class='title'>Convergente</div><div class='value'>{conv:,.2f}".replace(",", "X").replace(".", ",").replace("X",".") + f"</div><div class='delta'>{pct_conv:.1f}%</div></div>", unsafe_allow_html=True)
    kc3.markdown(f"<div class='kpi'><div class='title'>Divergente</div><div class='value'>{div:,.2f}".replace(",", "X").replace(".", ",").replace("X",".") + f"</div><div class='delta'>{pct(div):.1f}%</div></div>", unsafe_allow_html=True)
    status = "✅ Atingiu meta" if pct_conv >= meta_conv else "⚠️ Abaixo da meta"
    kc4.markdown(f"<div class='kpi'><div class='title'>Meta Convergência</div><div class='value'>{meta_conv:.0f}%</div><div class='delta'>{status}</div></div>", unsafe_allow_html=True)

    # ===== Agregações p/ gráficos =====
    grp = (df_dash.groupby([col_setor_macro, col_diaria, col_check], dropna=False)["_valor"].sum().reset_index())

    # ===== Gráfico 1 (Setor) — drill-through por clique =====
    st.markdown("#### 🔹 Por Setor (empilhado por Check)")
    df_setor = grp.groupby([col_setor_macro, col_check])["_valor"].sum().reset_index()
    ...
    click1 = st.plotly_chart(g1, use_container_width=True, key="g1", on_select="ignore")

    # ===== Gráfico 2 (Diária) — drill-through por clique =====
    st.markdown("#### 🔹 Por Diária Padrão (empilhado por Check)")
    df_diaria = grp.groupby([col_diaria, col_check])["_valor"].sum().reset_index()
    ...
    click2 = st.plotly_chart(g2, use_container_width=True, key="g2", on_select="ignore")

    # ===== Gráfico 3 (Pizza) =====
    st.markdown("#### 🔹 Distribuição Geral (Convergente × Divergente × Sem Relação)")
    df_pie = (df_dash
    ...
    g3.update_traces(textposition='inside', texttemplate='%{label}<br>%{percent:.1%}')
    ...
    if "sel_setor_from_chart" not in st.session_state: st.session_state.sel_setor_from_chart = None
    if "sel_diaria_from_chart" not in st.session_state: st.session_state.sel_diaria_from_chart = None
    ...
    if sel_setor != "(Todos)": df_detail = df_detail[df_detail["Setor Padrão"].astype(str) == sel_setor]
    if sel_diaria != "(Todos)": df_detail = df_detail[df_detail["Diária Padrão"].astype(str) == sel_diaria]
    ...
    fname = f"{safe_name(export_prefix)}_{now_stamp() if add_ts else ''}.xlsx".replace('__','.')
    ...
    st.download_button("🌐 Dashboard (HTML)", data=html_bytes, file_name="dashboard_diarias.html", mime="text/html")
    pdf_bytes = try_export_pdf(html_bytes)
    if pdf_bytes:
        st.download_button("🧾 Dashboard (PDF)", data=pdf_bytes, file_name="dashboard_diarias.pdf", mime="application/pdf")
    else:
    ...
    try:
        ...
        with c1: st.download_button("⬇️ Setor (PNG)", data=png1, file_name="grafico_setor.png", mime="image/png")
        with c2: st.download_button("⬇️ Diária (PNG)", data=png2, file_name="grafico_diaria.png", mime="image/png")
        with c3: st.download_button("⬇️ Distribuição (PNG)", data=png3, file_name="grafico_distribuicao.png", mime="image/png")
    except Exception:
        st.warning("Para exportar PNG, instale `kaleido`.")

    # salva Excel no /output
    DIR_OUT.mkdir(parents=True, exist_ok=True)
    with open(DIR_OUT / fname, "wb") as f: f.write(xlsb)
